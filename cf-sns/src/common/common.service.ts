...
export class CommonService {
  ...
  private async cursorPaginate<T extends BaseModel>(
    ...
    /**
     * where__likeCount__more_than 을 하고싶다면?
     *
     * where__title__iLike
     */
    const findOptions = this.composeFindOptions<T>(paginateDto);
    ...
    const lastItem =
      results.length > 0 && results.length === paginateDto.take
  ...
  private composeFindOptions<T extends BaseModel>(
     ...
     * 3) 필터 로직을 적용한다면 "__" 기준으로 split을 했을때 3개의 값으로 나뉘는지
     *    2개의 값으로 나뉘는지 확인한다.
    ...
    for (const [key, value] of Object.entries(paginateDto)) {
      // key -> where__id__less_than
      // value -> 1
    ...
    return {
      ...
      skip: paginateDto.page ? paginateDto.take * (paginateDto.page - 1) : null,
    };
  }
